# Primarily adapted from https://github.com/cloudposse/terraform-aws-eks-cluster
...
module "label" {
  ...
  attributes = compact(concat(module.this.attributes, ["gitlab-cluster"]))
...
resource "aws_eks_cluster" "gitlab-cluster" {
  count                     = local.enabled ? 1 : 0
  name                      = "gitlab-cluster"
...
# https://github.com/terraform-aws-modules/terraform-aws-eks/tree/v13.2.1/modules/node_groups
resource "aws_eks_node_group" "workers" {
  node_group_name = "gitlab-eks-ng"

  cluster_name  = "gitlab-cluster"
  node_role_arn = aws_iam_role.gitlab-cluster-role.arn
  subnet_ids    = ["subnet-0073fdc578c8421eb", "subnet-04925cb3aa035bf96", "subnet-0d04a93f0fcfcb3f0"]
  ...
  dynamic "remote_access" {
    for_each = "true" ? ["true"] : [] # clean up later with boolean variable set to true if need remote access
    content {
      ec2_ssh_key               = "gitlab-worker-kp"
  ...
  # For some reason terraform apply was throwing a strange error for these attachments
  # The role they need to belong to has them attached, so I don't think we need this depends_on or the attachment resources
#   depends_on = [
...
resource "aws_iam_openid_connect_provider" "default" {
  ...
  url   = join("", aws_eks_cluster.gitlab-cluster.*.identity.0.oidc.0.issuer)
