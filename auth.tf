...
locals {
  certificate_authority_data_list          = coalescelist(aws_eks_cluster.gitlab-cluster.*.certificate_authority, [[{ data : "" }]])
...
data "aws_eks_cluster" "eks" {
  count = local.enabled && var.apply_config_map_aws_auth ? 1 : 0
  name  = join("", aws_eks_cluster.gitlab-cluster.*.id)
}
...
# Would not work via terraform ---> As a workaround, edited the aws-auth ConfigMap manually
